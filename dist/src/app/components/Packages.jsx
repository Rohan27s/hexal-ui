...
import { FaRegCheckCircle, FaGem, FaRocket } from 'react-icons/fa';
var Packages = function () {
    var _a = useState('yearly'), billingCycle = _a[0], setBillingCycle = _a[1];
    var handleBillingChange = function (cycle) {
    ...
    return (<section id='pricing' className="relative py-20 bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 text-white overflow-hidden">
            ...
            <div className="relative z-10 max-w-7xl mx-auto">
                ...
                <div className="flex justify-center mb-12 relative">
                    <div className="inline-flex items-center relative p-1 bg-gray-800 rounded-full shadow-lg overflow-hidden">
                        ...
                        <div className={"absolute top-0 bottom-0 w-1/2 bg-gradient-to-r from-teal-600 to-blue-500 rounded-full transition-transform duration-300 ease-in-out ".concat(billingCycle === 'monthly' ? 'translate-x-0' : 'translate-x-full')}></div>

                        {['monthly', 'yearly'].map(function (cycle) { return (<button key={cycle} onClick={function () { return handleBillingChange(cycle); }} className={"px-6 py-2 z-10 font-medium text-lg relative rounded-full transition-all duration-300 focus:outline-none ".concat(billingCycle === cycle ? 'text-white' : 'text-gray-300')} style={{ minWidth: '130px' }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
                    {plans.map(function (plan, index) { return (<div key={index} className={"relative p-8 bg-white/10 backdrop-blur-lg rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 ease-in-out transform hover:scale-105 border border-transparent ".concat(plan.borderColor)}>
                            {plan.highlight && (<div className="absolute top-4 right-4 bg-yellow-500 text-gray-900 px-3 py-1 rounded-full text-xs font-bold">
                                    {plan.badge}
                                </div>)}
                            ...
                            <ul className="text-gray-300 text-sm space-y-3 mb-6">
                                {plan.features.map(function (feature, idx) { return (<li key={idx}>{feature}</li>); })}
                            </ul>
                            {billingCycle === 'yearly' && plan.title === 'Premium' && (<p className="text-sm text-gray-300 mb-4">Save 20% on yearly billing!</p>)}
